...
airecalidad_group <-  aire %>% 
  filter(!aireCalidad %in% c('contaminado', 'triste')) %>% 
...
ggplot(data = inundaciones_group, mapping = aes(x=inundaciones, y=n, fill=inundaciones)) +
  ...
  theme(
    ...
    plot.title = element_text(color="Black", size=20, face="bold.italic",hjust = 0.,# Ajuste horizontal
                              vjust = .5),
    plot.caption = element_text(size = 18),
  ...
  labs( x=NULL, 
        y=NULL,
        scale_fill_manual="Respuesta",
        title="¿Conoce lugares del barrio donde se estanque el agua \n o se inunde cuando llueve?", 
...
ggplot(data = sonora_group , #Dataset
  ...
  labs( x=NULL, 
        y=NULL,
        scale_fill_manual="Tipos",
        title="En su opinión ¿cuáles son las principales fuentes de \n contaminación  sonora en el barrio?", 
        caption="Percepciones sobre problemas \n ambientales y efectos sobre la salud.")
...
class(salud$problemasSaludFrecuentes)
salud_long <- cSplit(salud, "problemasSaludFrecuentes", ",", "long")
salud_long <-  salud_long %>% mutate(problemasSaludFrecuentes= as.character(problemasSaludFrecuentes)) %>% 
                            
  
  mutate(problemasSaludFrecuentes= case_when (problemasSaludFrecuentes %in% c('no sabe',
                                                                           'no conoce',
                                                                           'No sabe') ~ 'NSNC',
                                              problemasSaludFrecuentes  %in% c('Ninguno de los anteiores') ~ 'Ninguna',
                                              TRUE ~ problemasSaludFrecuentes) )
...
ggplot(data = coopCartoneros_group, mapping = aes(x=cartonerosCooperativa, y=n, fill=cartonerosCooperativa)) +
  ...
  labs( x=NULL, 
        y=NULL,
        scale_fill_manual="Respuesta",
        title="¿Conoce alguna cooperativa de cartoneros que \n trabaje en el barrio?", 
        caption="Higiene urbana y residuos")
